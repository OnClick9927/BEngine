...
public static class PrefabDocumentOperations
{
        ...
        Transform? parent = null, bool connectToPrefab = true)
    {
        ...
        CoreDocumentRegistration.ValidatePrefab(prefab.Document);
        var source = prefab.Document;
        var objectIds = source.GameObjects.ToDictionary(item => item.Id, _ => Guid.NewGuid());
        var sceneDocument = new SceneDocument
        {
            Id = Guid.NewGuid(),
            Name = source.Name,
        ...
        var temporary = SceneDocumentConverter.Shared.ToScene(sceneDocument);
    ...
    public static GameObject LoadContents(PrefabAsset prefab)
    {
        ArgumentNullException.ThrowIfNull(prefab);
        CoreDocumentRegistration.ValidatePrefab(prefab.Document);
        var ids = prefab.Document.GameObjects.ToDictionary(item => item.Id, item => item.Id);
        var sceneDocument = new SceneDocument
        {
            Id = Guid.NewGuid(),
            Name = prefab.name,
            GameObjects = [.. prefab.Document.GameObjects.Select(item =>
                CloneForInstance(item, prefab.assetId, ids, connected: false, preserveIds: true))]
        };
        var temporary = SceneDocumentConverter.Shared.ToScene(sceneDocument);
        var root = temporary.Find(prefab.Document.Root) ??
    ...
    private static GameObjectDocument CloneForInstance(GameObjectDocument source, Guid assetId,
        IReadOnlyDictionary<Guid, Guid> objectIds, bool connected, bool preserveIds = false)
    {
        var instance = new GameObjectDocument
        {
            ...
            Transform = new TransformDocument
            {
                ...
                LocalPosition = new FixedVector2Document(source.Transform.LocalPosition.ToVector2()),
                LocalRotation = source.Transform.LocalRotation,
                LocalScale = new FixedVector2Document(source.Transform.LocalScale.ToVector2()),
                Fields = new Dictionary<string, string>(source.Transform.Fields, StringComparer.Ordinal)
            }
        };
        instance.Components.AddRange(source.Components.Select(component => new ComponentDocument
        {
