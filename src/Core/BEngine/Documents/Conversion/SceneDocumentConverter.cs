...
{
    private static readonly Type[] SupportedObjectTypes = [typeof(Scene)];
    private readonly ComponentTypeRegistry _types = new();

    internal static SceneDocumentConverter Shared { get; } = new();

    public Type DocumentType => typeof(SceneDocument);
    ...
    public BObject ToBObject(Document document, DocumentConversionContext context) =>
        ToScene((SceneDocument)document, context.Services);

    public Document FromBObject(BObject value, DocumentConversionContext context) =>
        FromScene((Scene)value);

    public void UnregisterAssembly(Assembly assembly) => _types.UnregisterAssembly(assembly);

    internal static SceneDocument FromScene(Scene scene)
    {
    ...
    internal static GameObjectDocument FromGameObject(GameObject gameObject)
    {
        SerializationCallbackUtility.BeforeSerialize(gameObject.transform);
        var document = new GameObjectDocument
        {
            Id = gameObject.Id,
            ...
            Transform = new TransformDocument
            {
                ...
                LocalPosition = new FixedVector2Document(gameObject.transform.localPosition),
                LocalRotation = gameObject.transform.localRotation.ToString(),
                LocalScale = new FixedVector2Document(gameObject.transform.localScale),
        ...
        foreach (var component in gameObject.components.Where(component => component is not Transform))
        {
            ...
            document.Components.Add(new ComponentDocument
            {
                Id = component.Id,
    ...
    private void RestoreTransformType(GameObject gameObject, TransformDocument document)
    {
        if (string.IsNullOrWhiteSpace(document.Type) || document.Type == typeof(Transform).FullName) return;
        var transformType = _types.Resolve(document.Type);
    ...
    private static void RestoreTransform(Transform transform, TransformDocument document)
    {
    ...
    private void RestoreComponents(GameObject gameObject, IEnumerable<ComponentDocument> documents)
    {
        foreach (var document in documents)
        {
            var type = _types.Resolve(document.Type);
            Component component;
            if (type is null)
            {
