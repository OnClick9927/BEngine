...
- Create a custom editor theme with `Assets/Create/GUI/GUISkin`, or clone the active theme with `Edit/Preferences > General > Theme > New`. GUISkin assets use `.guiskin.yaml` and derive from `BAsset` in `BEngine.Editor`.
...
- Open `Edit/Preferences > General`. The Theme list always shows built-in `Light`, `Dark`, and `Classic`, followed by every project `.guiskin.yaml` asset. Built-ins are stable `GUISkin` objects that can be selected through their ObjectField and inspected, but their Inspector is read-only and they cannot be deleted.
- Click a Theme row's ObjectField to set `Selection.activeObject` and show that Skin in Inspector. `New` clones the active Skin into the current Project folder and starts rename; edit its palette, built-in style slots, and `customStyles`, then use the Inspector Apply bar to persist it.
...
- A `GUISkin` owns the palette and all standard/editor slots, including label, box, button, toggle, text fields, sliders/scrollbars, toolbar controls, dock tabs, menu items, tree/hierarchy rows, foldout, help box, and Inspector styles. `customStyles` is serialized and `FindStyle`/`GetStyle` lookup is case-insensitive; use names scoped as `PackageName/ControlName` to avoid collisions.
...
- Read the active Skin through `GUI.skin`. Use `EditorAppearance.SetSkin(skin)` for an editor-wide change because it synchronizes `EditorAppearance.palette`, theme state, appearance notifications, and repaint; direct `GUI.skin` assignment is reserved for host internals and focused tests.
