...
public sealed class Animator : Behaviour
{
    ...
    private void DispatchEvents(AnimationClip clip, Fix64 previous, Fix64 current)
    {
        ...
        foreach (var behaviour in gameObject.GetComponents<MonoBehaviour>().Where(item => item.enabled))
        {
            var method = behaviour.GetType().GetMethod(animationEvent.functionName,
                BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic);
            if (method?.GetParameters().Length == 0) method.Invoke(behaviour, null);
            else if (method?.GetParameters() is [{ ParameterType: var type }] && type == typeof(AnimationEvent)) method.Invoke(behaviour, [animationEvent]);
        }
    }
...
internal sealed class AnimationRuntimeSystem : ISceneRuntimeSystem
{
    public int order => 50;
    public string packageId => "com.bengine.animation";
    public void Update(Scene scene, Fix64 deltaTime)
    {
        foreach (var animator in scene.gameObjects.Where(item => item.activeInHierarchy)
                     .SelectMany(item => item.GetComponents<Animator>()).OrderBy(item => item.Id)) animator.Tick(deltaTime);
        foreach (var animation in scene.gameObjects.Where(item => item.activeInHierarchy)
                     .SelectMany(item => item.GetComponents<Animation>()).OrderBy(item => item.Id)) animation.Tick(deltaTime);
    }
}
